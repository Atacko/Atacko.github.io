...
const ORIGINAL_GAME_WIDTH = 550; 
const ORIGINAL_GAME_HEIGHT = 335; 

const CENTER_PADDING = 20; 

const MAZE_PADDING = 10; 

function resizeGame() {
  const availableWidth = window.innerWidth - CENTER_PADDING;
  const availableHeight = window.innerHeight - CENTER_PADDING;
  ...
  gameElement.style.transform = `scale(${scaleFactor})`;
}
...
const walls = [
  ...
  { column: 0, row: 0, horizontal: false, length: 9.05 }, 
  { column: 0, row: 9, horizontal: true, length: 10 },
  { column: 10, row: 0, horizontal: false, length: 9.05 }, 
  ...
  x: wall.column * (pathW + wallW) + MAZE_PADDING,
  y: wall.row * (pathW + wallW) + MAZE_PADDING,
...
const holes = [
  ...
  x: hole.column * (wallW + pathW) + (wallW / 2 + pathW / 2) + MAZE_PADDING,
  y: hole.row * (wallW + pathW) + (wallW / 2 + pathW / 2) + MAZE_PADDING
}));
...
function resetGame() {
  ...
  balls = [
    ...
    x: ball.column * (wallW + pathW) + (wallW / 2 + pathW / 2) + MAZE_PADDING,
    y: ball.row * (wallW + pathW) + (wallW / 2 + pathW / 2) + MAZE_PADDING,
    velocityX: 0,
    velocityY: 0
  }));
...
function main(timestamp) {
  ...
  try {
    ...
    if (
      balls.every(
        (ball) => distance2D(ball, { x: 350 / 2 + MAZE_PADDING, y: 315 / 2 + MAZE_PADDING }) < 65 / 2
      )
    ) {
